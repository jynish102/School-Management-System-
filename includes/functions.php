...
function get_users($args =array(), $type = 'object')
{
    ...
    if(!empty($args))
    {
        ...
        if($condition_ar > 0){
            $condition = " WHERE 
            " .implode(" AND ", $condition_ar);
        }
    }
    $query = mysqli_query($db_conn, "SELECT * FROM account $condition");
...
{
    global $db_conn;
    $query = mysqli_query($db_conn, "SELECT * FROM account WHERE id = $user_id");
...
/*function get_users($args = array(), $type = 'object')
{
        global $db_conn;
        $condition = "";
        $condition_ar = array(); // initialize array

        if (!empty($args)) {
            foreach ($args as $k => $v) {
                $v = mysqli_real_escape_string($db_conn, (string)$v); // prevent SQL injection
                $condition_ar[] = "$k = '$v'";
            }
